...
def main():
    ...
    for row in rdr:
        ...
        if prev_href == href_act and hr != href_act:
            if row_acc:
                rows_bytes.append(bytes_to_pixels_row(row_acc, len(data_cols), args.byte_order))
            row_acc = []
    ...
    width = len(data_cols)  # Width based on the number of data channels (D0..D7)
    height = len(rows_bytes)  # Height based on the number of rows (HREF-high)

    # Assemble final frame buffer (2*width bytes per row), pad/crop to height
    row_len = 2 * width
    while len(rows_bytes) < height:
        rows_bytes.append(np.zeros(row_len, dtype=np.uint8))
    rows_bytes = rows_bytes[:height]
    frame = np.concatenate(rows_bytes, axis=0)

    img = buffer_to_image(frame.tobytes(), width, height,
